 ...
 *
 * Configuration settings for the MX53-UMoBo board.
 *
...
#define CONFIG_FASTBOOT_MANUFACTURER_STR  "U-MoBo"
#define CONFIG_FASTBOOT_PRODUCT_NAME_STR "i.mx53 U-MoBo"
...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
		...
		"bootargs_base=console=ttymxc3,115200 init=/init "	\
			"androidboot.console=ttymxc3 "			\
			"androidboot.hardware=freescale "		\
			"fbmem=12M,12M "				\
			"video=mxcfb0:dev=lcd,SAMSUNG-LMS700,if=RGB24 "	\
			"video=mxcfb1:dev=vga,VGA-SVGA,if=RGB24\0"	\
		"bootcmd_eMMC=setenv bootargs ${bootargs_base} "	\
			"da9052_i2c_addr=${da9052_i2c_addr};"		\
			"mmc dev 1;"					\
			"mmc read ${loadaddr} 0x800 0x2000;"		\
			"mmc read ${rd_loadaddr} 0x3000 0x400;"		\
		...
		"bootcmd_NFS=dhcp;nfs ${loadaddr} ${serverip}:${nfskernel}; "	\
			...
			"bootm\0"						\
		"bootcmd_SD=setenv bootargs ${bootargs_base} "		\
			"da9052_i2c_addr=${da9052_i2c_addr};"		\
			"mmc dev 0;"					\
			"mmc read ${loadaddr} 0x800 0x2000;"		\
			"mmc read ${rd_loadaddr} 0x3000 0x400;"		\
...
/* flags to be investigated !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		"flags=quiet lpj=4980736\0"			\
		"ion=128M,64M fbmem=12M,12M "			\
		"gpu_memory=128M vmalloc=576M "			\
*/
