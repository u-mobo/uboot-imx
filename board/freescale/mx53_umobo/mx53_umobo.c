...
static struct fb_videomode lcd_wvga = {
...
#ifdef CONFIG_UMOBO_BOARD
extern void setup_umobo_board(void);
#endif
...
{
	/* UART4 RXD */
	mxc_request_iomux(MX53_PIN_KEY_ROW0, IOMUX_CONFIG_ALT4);
	mxc_iomux_set_pad(MX53_PIN_KEY_ROW0, 0x1E4);
	mxc_iomux_set_input(MUX_IN_UART4_IPP_UART_RXD_MUX_SELECT_INPUT, 0x1);

	/* UART4 TXD */
	mxc_request_iomux(MX53_PIN_KEY_COL0, IOMUX_CONFIG_ALT4);
	mxc_iomux_set_pad(MX53_PIN_KEY_COL0, 0x1E4);
}
...
int da9053_i2c_startup_reset(void)
{
	...
	/* just issue a dummy read */
	i2c_read(DA9052_I2C_ADDR, 0xff, 1, value, 2);
	return 0;
}
...
void spi_io_init(struct imx_spi_dev_t *dev)
{
	...
	case CSPI1_BASE_ADDR:
		...
		if (dev->ss == 0) {
			...
			mxc_request_iomux(MX53_PIN_KEY_COL2, IOMUX_CONFIG_ALT4);
			mxc_iomux_set_pad(MX53_PIN_KEY_COL2, 0x104);
			mxc_iomux_set_input(
...
static void setup_fec(void)
{
	...
	/* phy reset: gpio5-20 */
	mxc_request_iomux(MX53_PIN_CSI0_DATA_EN, IOMUX_CONFIG_ALT1);
...
void lcd_enable(void)
{
	...
	*/
	...
	/* FIXME: enable backlight? */

	ret = ipuv3_fb_init(&lcd_wvga, di, IPU_PIX_FMT_RGB24,
...
void panel_info_init(void)
{
	...
	panel_info.vl_col = lcd_wvga.xres;
	panel_info.vl_row = lcd_wvga.yres;
...
#define USBCMD 0x140
	...
	gd->bd->bi_arch_number = MACH_TYPE_MX53_UMOBO;	/* board id for linux */
	...
	/* VDDIO_FEC from DA9052: setup_fec moved to board_late_init() */
...
#endif
#ifdef CONFIG_MXC_FEC
	/* we had to wait for VDDIO_FEC from DA9052 */
	setup_fec();
#endif
#endif
	return 0;
